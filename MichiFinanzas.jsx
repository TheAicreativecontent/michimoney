...
const T = {
  es: {
    ...
    this_month:"Tu mes · {month}", fixed_label:"🏠 Fijos", var_label:"🛍️ Variables", sav_label:"🐷 Ahorro", ring_gastos:"Gastos", ring_ahorro:"Ahorro", ring_inversion:"e inversión", over_budget:"te has pasado este mes", acc_savings:"ahorro acumulado total", this_month_savings:"ahorrado este mes",
  ...
  en: {
    ...
    this_month:"This month · {month}", fixed_label:"🏠 Fixed", var_label:"🛍️ Variable", sav_label:"🐷 Savings", ring_gastos:"Expenses", ring_ahorro:"Savings", ring_inversion:"& investment", over_budget:"over budget this month", acc_savings:"total accumulated savings", this_month_savings:"saved this month",
          ...
          <div className="mf-rings">
            <div className="mf-ring" onClick={() => setTab("fijos")} style={{ cursor: "pointer" }}>
              <Battery value={fixedM} max={budgets.fixed || 1} color="#6FBEEF" track="#DCF0FC" overColor="#E8453C" goalPct={goalFixedPct} size={96}
                ...
                label={<><div style={{ fontSize: 10, lineHeight: 1.2, opacity: 0.7 }}>{t("ring_gastos")}</div><div>{t("nav_fijos")}</div></>} />
            </div>
            <div className="mf-ring" onClick={() => setTab("variables")} style={{ cursor: "pointer" }}>
              <Battery value={varSpent} max={budgets.variable || 1} color="#FF8FA8" track="#FFD9E3" overColor="#E8453C" goalPct={goalVarPct} size={96}
                ...
                label={<><div style={{ fontSize: 10, lineHeight: 1.2, opacity: 0.7 }}>{t("ring_gastos")}</div><div>{t("nav_variables")}</div></>} />
            </div>
          ...
          <div className="mf-rings">
            <div className="mf-ring" onClick={() => setTab("fijos")} style={{ cursor: "pointer" }}>
              ...
              <div className="lab" style={{ textAlign: "center", lineHeight: 1.3 }}>
                ...
                <div>{t("nav_fijos")}{fixedM > budgets.fixed && budgets.fixed > 0 ? <b style={{ color: "#E8453C" }}> +{Math.round(((fixedM - budgets.fixed) / budgets.fixed) * 100)}%</b> : null}</div>
              </div>
            </div>
            <div className="mf-ring" onClick={() => setTab("variables")} style={{ cursor: "pointer" }}>
              ...
              <div className="lab" style={{ textAlign: "center", lineHeight: 1.3 }}>
                <div style={{ fontSize: 9, opacity: 0.7 }}>{t("ring_gastos")}</div>
                <div>{t("nav_variables")}{varSpent > budgets.variable && budgets.variable > 0 ? <b style={{ color: "#E8453C" }}> +{Math.round(((varSpent - budgets.variable) / budgets.variable) * 100)}%</b> : null}</div>
              </div>
            </div>
            <div className="mf-ring" onClick={() => setTab("resumen")} style={{ cursor: "pointer" }}>
...
function Ayuda({ setTab }) {
  ...
  const faqs = LANG === "en" ? [
    ...
    { section: "🏠 Fixed expenses", items: [
      ...
      { q: "What do D, S, Q, M, T, A mean?", a: "Daily, Weekly, Bi-weekly, Monthly, Quarterly, Annual — the frequency you pay that expense." },
    ]},
