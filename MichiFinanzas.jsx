...
const T = {
  es: {
    ...
    this_month:"Tu mes · {month}", fixed_label:"🏠 Fijos", var_label:"🛍️ Variables", sav_label:"🐷 Ahorro", over_budget:"te has pasado este mes", acc_savings:"ahorro acumulado total", this_month_savings:"ahorrado este mes",
    missions_title:"🎯 Misiones de hoy", mission_log:"Registra tu día", mission_nospend:"Día sin gastos variables", mission_under:"Bajo presupuesto variable este mes",
  ...
  en: {
    ...
    this_month:"This month · {month}", fixed_label:"🏠 Fixed", var_label:"🛍️ Variable", sav_label:"🐷 Savings", over_budget:"over budget this month", acc_savings:"total accumulated savings", this_month_savings:"saved this month",
    missions_title:"🎯 Today's missions", mission_log:"Log your day", mission_nospend:"No variable expenses day", mission_under:"Under variable budget this month",
...
.mf-tsub{font-size:11px; color:var(--ink-soft); font-weight:700; margin-top:2px; line-height:1.3; min-height:29px; animation:mf-tip .5s ease both;}
...
.mf-bubble{flex:1; background:var(--lav-soft); border-radius:18px 18px 18px 6px; padding:12px 14px; font-weight:700; font-size:14px; position:relative; line-height:1.35;}
...
const FX_FALLBACK = { EUR: 1, USD: 1.16, THB: 37.7, BTC: 1 / 60000 };
...
const SYM = { THB: "฿", USD: "$", EUR: "€", BTC: "₿" };
// BTC tiene 8 decimales (satoshis); las demás monedas, 2
const ccyDec = (c) => (c === "BTC" ? 8 : 2);
...
export default function App() {
  ...
  useEffect(() => {
    if (!loaded || backedUpToday.current) return;
    ...
    shareOrDownloadBackup(settings, fixed, txns, income, patrimonio, patrimonioHistory, true).then(() => {
      setLastBackup(getLastBackup());
      toastMsg(t("toast_auto_backup"));
    });
  }, [loaded, settings, fixed, txns, income, patrimonio, patrimonioHistory]);
  ...
  }, [settings, fixed, txns, income, patrimonio, patrimonioHistory]);
  ...
  const loadFx = useCallback(() => {
    ...
    withT("https://api.frankfurter.app/latest?from=EUR&to=USD,THB", 7000)
      .then((r) => r.ok ? r.json() : Promise.reject())
      .then((d) => { if (!d || !d.rates || !d.rates.USD || !d.rates.THB) throw 0; return { USD: d.rates.USD, THB: d.rates.THB, date: d.date }; })
      .catch(() => withT("https://open.er-api.com/v6/latest/EUR", 7000)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { if (!d || !d.rates || !d.rates.USD || !d.rates.THB) throw 0; return { USD: d.rates.USD, THB: d.rates.THB, date: (d.time_last_update_utc || "").slice(5, 16) }; }))
      .then((res) => Promise.all([res, btcPromise]))
      .then(([res, btc]) => { setFxRates({ EUR: 1, USD: res.USD, THB: res.THB, BTC: btc }); setFxUpdated(res.date || ""); setFxLive(true); })
  ...
  const gamification = useMemo(() => {
    ...
    let xp = 0;
    xp += real.length * 6;
    xp += txns.filter((t) => t.noSpend).length * 12;
    if (incomeThisMonth > 0) xp += 10;
    xp += Math.min(fixed.length, 6) * 5;
    // streak: días seguidos con actividad (gasto o día sin gastos)
    const daySet = new Set(txns.map((t) => t.date));
    ...
    // ahorro acumulado (meses con actividad + actual)
    ...
    const noSpendDays = txns.filter((t) => t.noSpend).length;
    const cats = new Set(real.map((t) => t.cat));
    const underThisMonth = incomeThisMonth > 0 && varSpentIn(txns, curYm) <= budgets.variable;
    const level = Math.floor(xp / 100) + 1;
    const into = xp % 100;
    const badges = [
      { id: "first", ic: "🌱", nm: t("badge_first"), on: real.length >= 1 },
      { id: "s3", ic: "🔥", nm: t("badge_s3"), on: streak >= 3 },
      { id: "s7", ic: "⚡", nm: t("badge_s7"), on: streak >= 7 },
      { id: "s14", ic: "🌟", nm: t("badge_s14"), on: streak >= 14 },
      { id: "s30", ic: "👑", nm: t("badge_s30"), on: streak >= 30 },
      { id: "ns1", ic: "🍃", nm: t("badge_ns1"), on: noSpendDays >= 1 },
      { id: "ns5", ic: "🧘", nm: t("badge_ns5"), on: noSpendDays >= 5 },
      { id: "under", ic: "🎯", nm: t("badge_under"), on: underThisMonth },
      { id: "acc100", ic: "🐷", nm: t("badge_acc100"), on: acc >= 100 },
      { id: "acc500", ic: "💰", nm: t("badge_acc500"), on: acc >= 500 },
      { id: "acc1000", ic: "🏆", nm: t("badge_acc1000"), on: acc >= 1000 },
      { id: "cats", ic: "🌈", nm: t("badge_cats"), on: cats.size >= (settings.categories || []).length && (settings.categories || []).length > 0 },
    ];
    return { xp, level, into, streak, acc, noSpendDays, badges, badgeCount: badges.filter((b) => b.on).length };
  }, [txns, fixed, settings, fixedM, budgets, curYm, income, fxRates, incomeThisMonth]);

  // confeti al subir de nivel o desbloquear logro
  const pl = useRef(null), pb = useRef(null);
  useEffect(() => {
    if (!loaded) return;
    if (pl.current !== null && gamification.level > pl.current) { setBurst(Date.now()); toastMsg(t("toast_level", { n: gamification.level })); }
    else if (pb.current !== null && gamification.badgeCount > pb.current) { setBurst(Date.now()); toastMsg(t("badge_unlocked")); }
    pl.current = gamification.level; pb.current = gamification.badgeCount;
  }, [gamification.level, gamification.badgeCount, loaded, toastMsg]);
  ...
  // frases rotativas del header: el subtitulo base + consejos de uso, cambian cada pocos segundos
  const [tipIdx, setTipIdx] = useState(0);
  useEffect(() => {
    const iv = setInterval(() => setTipIdx((i) => i + 1), 6500);
  ...
  const ctx = { settings, setSettings, fixed, setFixed, txns, addTxn, addNoSpend, delTxn, editTxn, income, setIncome, addIncome, incomeThisMonth, budgets, fixedM, cur, curYm, varSpentCurYm, gamification, setTab, toastMsg, onReset, fx, lastBackup, doManualBackup, applyBackup, lang, changeLang, patrimonio, setPatrimonio, patrimonioHistory, setPatrimonioHistory };
  const NAV = [
      ...
      <div className="mf-phone">
        <div className="mf-header"><div className="mf-hrow">
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div>{(() => {
              const phrases = [t("subtitle"), ...((T[LANG] && T[LANG].tips) || T.en.tips)];
              return (<><div className="mf-title">Michi<b>{t("title_suffix")}</b></div><div className="mf-tsub" key={tipIdx}>{phrases[tipIdx % phrases.length]}</div></>);
            })()}</div>
          ...
          <div className="mf-chip">
            <button className="mf-mini" onClick={() => setTab("logros")}>🔥 {gamification.streak}</button>
...
/* ---------- INICIO ---------- */
function Inicio({ settings, incomeThisMonth, budgets, fixedM, txns, cur, curYm, varSpentCurYm, gamification, setTab, lastBackup }) {
  ...
  const missions = [
    { t: t("mission_log"), done: txns.some((tx) => tx.date === todayISO()), xp: 6 },
    { t: t("mission_nospend"), done: txns.some((tx) => tx.date === todayISO() && tx.noSpend) && !txns.some((tx) => tx.date === todayISO() && !tx.noSpend), xp: 12 },
    { t: t("mission_under"), done: incomeThisMonth > 0 && varSpent <= budgets.variable, xp: 15 },
  ];
  ...
  return (
    <div className="mf-page">
      {!noIncome && (() => {
        ...
        const pct = isNeg ? 0 : Math.min(1, savingsNow / goal);
        const overPct = !isNeg && savingsNow > goal ? Math.min(1, (savingsNow - goal) / goal) : 0;
        return (
          <div className="mf-hero">
            <div className="lab">{t("savings_label")}</div>
            <div className="v mf-num" style={{ color: isNeg ? "#E8453C" : undefined }}>
              {isNeg ? "−" : ""}{money(Math.abs(Math.round(savingsNow)), cur)}
            </div>
            <div style={{ width: "100%", marginTop: 8 }}>
              <div style={{ position: "relative", height: 10, borderRadius: 99, background: "rgba(255,255,255,0.25)", overflow: "hidden" }}>
                {isNeg
      ...
      <div className="mf-card">
        <div className="mf-mascotwrap"><Mascot mood={mood} size={86} /><div className="mf-bubble">{bubble}</div></div>
        <div className="mf-xprow">
          <span className="mf-lvl">{t("level_abbr", { n: gamification.level })}</span>
          <div className="mf-xpbar"><div className="mf-xpfill" style={{ width: `${gamification.into}%` }} /></div>
          <span style={{ fontFamily: "Baloo 2", fontWeight: 800, fontSize: 12, color: "var(--ink-soft)" }}>{gamification.into}/100</span>
        </div>
        ...
        {settings.chartMode === "batteries" ? (
          <div className="mf-rings">
            <div className="mf-ring" onClick={() => setTab("fijos")} style={{ cursor: "pointer" }}>
              <Battery value={fixedM} max={budgets.fixed || 1} color="#6FBEEF" track="#DCF0FC" overColor="#E8453C" goalPct={goalFixedPct}
                center={<div className="mf-ringval" style={{ fontSize: 12 }}>{fmt(Math.round(fixedM))}</div>} label={t("fixed_label")} />
            </div>
            <div className="mf-ring" onClick={() => setTab("variables")} style={{ cursor: "pointer" }}>
              <Battery value={varSpent} max={budgets.variable || 1} color="#FF8FA8" track="#FFD9E3" overColor="#E8453C" goalPct={goalVarPct}
                center={<div className="mf-ringval" style={{ fontSize: 12 }}>{fmt(Math.round(varSpent))}</div>} label={t("var_label")} />
            </div>
            <div className="mf-ring" onClick={() => setTab("resumen")} style={{ cursor: "pointer" }}>
              <Battery value={Math.max(0, savingsNow)} max={budgets.savings || 1} color="#9AE6B4" track="#E1F7E9" overColor="#1E9E5C"
                center={<div className="mf-ringval" style={{ fontSize: 12 }}>{fmt(Math.round(Math.max(0, savingsNow)))}</div>} label={t("sav_label")} />
            </div>
          </div>
        ) : (
          <div className="mf-rings">
            <div className="mf-ring" onClick={() => setTab("fijos")} style={{ cursor: "pointer" }}>
              <Ring value={fixedM} max={budgets.fixed || 1} color="#6FBEEF" track="#DCF0FC" overColor="#E8453C" goalPct={goalFixedPct}
                center={<div className="mf-ringval">{fmt(Math.round(fixedM))}<br/><small>/{fmt(Math.round(budgets.fixed))}</small></div>} />
              <span className="lab">{t("fixed_label")} {fixedM > budgets.fixed && budgets.fixed > 0 ? <b style={{ color: "#E8453C" }}>· +{Math.round(((fixedM - budgets.fixed) / budgets.fixed) * 100)}%</b> : null}</span>
            </div>
            <div className="mf-ring" onClick={() => setTab("variables")} style={{ cursor: "pointer" }}>
              <Ring value={varSpent} max={budgets.variable || 1} color="#FF8FA8" track="#FFD9E3" overColor="#E8453C" goalPct={goalVarPct}
                center={<div className="mf-ringval">{fmt(Math.round(varSpent))}<br/><small>/{fmt(Math.round(budgets.variable))}</small></div>} />
              <span className="lab">{t("var_label")} {varSpent > budgets.variable && budgets.variable > 0 ? <b style={{ color: "#E8453C" }}>· +{Math.round(((varSpent - budgets.variable) / budgets.variable) * 100)}%</b> : null}</span>
            </div>
            <div className="mf-ring" onClick={() => setTab("resumen")} style={{ cursor: "pointer" }}>
              <Ring value={Math.max(0, savingsNow)} max={budgets.savings || 1} color="#9AE6B4" track="#E1F7E9" overColor="#1E9E5C"
                center={<div className="mf-ringval">{fmt(Math.round(Math.max(0, savingsNow)))}<br/><small>/{fmt(Math.round(budgets.savings))}</small></div>} />
              <span className="lab">{t("sav_label")} {savingsNow > budgets.savings && budgets.savings > 0 ? <b style={{ color: "#1E9E5C" }}>· +{Math.round(((savingsNow - budgets.savings) / budgets.savings) * 100)}%</b> : null}</span>
            </div>
          </div>
        )}
        {budgets.variable > 0 && (() => {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, width: "100%" }}>
            ...
            <div>
              <div className="big" style={{ color: savingsNow >= 0 ? "var(--good)" : "var(--warn)" }}>
                {settings.rolloverSavings ? money(Math.round(gamification.acc), cur) : money(Math.round(savingsNow), cur)}
              </div>
      ...
      <div className="mf-card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div className="mf-h3" style={{ margin: 0 }}>{t("missions_title")}</div>
          <span style={{ fontFamily: "Baloo 2", fontWeight: 800, fontSize: 12, color: "var(--ink-soft)" }}>+{missions.filter((m) => m.done).reduce((a, m) => a + m.xp, 0)} XP</span>
        </div>
        <div style={{ marginTop: 4 }}>{missions.map((m, i) => (
          <div className="mf-quest" key={i}>
            <div className={`mf-qbox ${m.done ? "done" : ""}`}>{m.done ? "✓" : ""}</div>
            <div className={`mf-qtxt ${m.done ? "done" : ""}`}>{m.t}</div>
            <div className="mf-qxp">+{m.xp}</div>
          </div>
        ))}</div>
      </div>
...
function Añadir({ settings, addTxn, addNoSpend, addIncome, cur, setTab, toastMsg, fx }) {
  ...
  return (
    <div className="mf-page">
      ...
      {mode === "gasto" && (<>
        <div className="mf-card">
          <div className="mf-seg" style={{ justifyContent: "center", marginBottom: 8 }}>
            {["THB", "USD", "EUR", "BTC"].map((c) => (
              <button key={c} className={`mf-segp ${ccy === c ? "on" : ""}`} onClick={() => setCcy(c)}>{SYM[c]} {c}</button>
            ))}
          </div>
      ...
      {mode === "ingreso" && (<>
        <div className="mf-card">
          <div className="mf-seg" style={{ justifyContent: "center", marginBottom: 8 }}>
            {["EUR", "USD", "THB", "BTC"].map((c) => (
              <button key={c} className={`mf-segp ${incCcy === c ? "on" : ""}`} onClick={() => setIncCcy(c)}>{SYM[c]} {c}</button>
            ))}
          </div>
        ...
        {monthTx.length ? monthTx.map((tx) => {
          ...
          if (draft && draft.id === tx.id) return (
            <div key={tx.id} style={{ padding: "11px 0", borderBottom: "1px dashed var(--pink-soft)" }}>
              <div className="mf-seg" style={{ marginBottom: 8 }}>{["THB", "USD", "EUR", "BTC"].map((cc) => (
                <button key={cc} className={`mf-segp ${draft.ccy === cc ? "on" : ""}`} onClick={() => setDraft((d) => ({ ...d, ccy: cc }))}>{SYM[cc]} {cc}</button>
              ))}</div>
    ...
    <div className="mf-page">
      ...
      <div className="mf-card">
        ...
        <div className="mf-field"><label>{t("currency_label")}</label>
          <div className="mf-seg">{["THB", "USD", "EUR", "BTC"].map((c) => (
            <button key={c} className={`mf-segp ${ccy === c ? "on" : ""}`} onClick={() => setCcy(c)}>{SYM[c]} {c}</button>
          ))}</div>
        </div>
          ...
          <div key={f.id} style={{ padding: "11px 0", borderBottom: "1px dashed var(--pink-soft)" }}>
            {editingId === f.id ? (
              <>
                ...
                <div className="mf-field"><label>{t("currency_label")}</label>
                  <div className="mf-seg">{["THB", "USD", "EUR", "BTC"].map((c) => (
                    <button key={c} className={`mf-segp ${(f.cur || "EUR") === c ? "on" : ""}`} onClick={() => updateFixed(f.id, "cur", c)}>{SYM[c]} {c}</button>
                  ))}</div>
                </div>
...
/* ---------- RESUMEN ---------- */
function Resumen({ settings, income, incomeThisMonth, txns, budgets, fixedM, cur, curYm, varSpentCurYm, gamification, fx }) {
  const varSpent = varSpentCurYm;
  const savingsNow = incomeThisMonth - fixedM - varSpent;
    ...
    <div className="mf-page">
      ...
      <div className="mf-card">
        ...
        <div className="mf-grid2">
          ...
          <div className="mf-stat"><div className="v mf-num" style={{ color: "var(--good)" }}>{money(Math.round(gamification.acc), cur)}</div><div className="l">{t("acc_savings_label")}</div></div>
...
/* ---------- LOGROS ---------- */
function Logros({ gamification }) {
  const g = gamification;
  return (
    <div className="mf-page">
      <h2 className="mf-h2">{t("logros_title")}</h2>
      <div className="mf-card"><div className="mf-grid2">
        <div className="mf-stat"><div className="v mf-num">{t("level_abbr", { n: g.level })}</div><div className="l">{t("level_label")}</div></div>
        <div className="mf-stat"><div className="v mf-num">{g.xp}</div><div className="l">{t("xp_label")}</div></div>
        <div className="mf-stat"><div className="v mf-num">🔥 {g.streak}</div><div className="l">{t("streak_label")}</div></div>
        <div className="mf-stat"><div className="v mf-num">{g.badgeCount}/{g.badges.length}</div><div className="l">{t("badges_label")}</div></div>
      </div></div>
      <div className="mf-card">
        <div className="mf-h3">{t("badges_label")}</div>
        <div className="mf-badges">{g.badges.map((b) => (
          <div className={`mf-badge ${b.on ? "on" : ""}`} key={b.id}><div className="ic">{b.ic}</div><div className="nm">{b.nm}</div></div>
        ))}</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="mf-page" style={{ padding: "16px 18px 100px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: 18, marginBottom: 4 }}>
        <div style={{ width: "70%", maxWidth: 260, borderRadius: 22, border: "4px solid var(--lav)", boxShadow: "0 0 0 3px var(--pink-soft), 0 4px 18px rgba(180,120,220,0.18)", overflow: "hidden", lineHeight: 0 }}>
...
/* ---------- AJUSTES ---------- */
function Ajustes({ settings, setSettings, cur, toastMsg, onReset, lastBackup, doManualBackup, applyBackup, setTab, lang, changeLang }) {
    ...
    <div className="mf-page">
      ...
      <div className="mf-card">
        ...
        <div className="mf-seg" style={{ justifyContent: "center", marginBottom: 12 }}>
          <button className={`mf-segp ${s.chartMode !== "batteries" ? "on" : ""}`} onClick={() => set("chartMode", "donuts")}>{t("donuts_btn")}</button>
...
function fxCalc(src, raw, rates) {
  const order = ["THB", "USD", "EUR", "BTC"];
  const num = parseFloat(String(raw).replace(",", "."));
  if (raw === "" || isNaN(num)) { const o = { THB: "", USD: "", EUR: "", BTC: "" }; o[src] = raw; return o; }
  const eur = num / rates[src];
  const o = {};
  order.forEach((c) => { o[c] = c === src ? raw : fxRound(eur * rates[c], c === "BTC" ? 8 : 2); });
  return o;
}
const FX_META = [
...
/* ---------- BTC ---------- */
function BtcPage({ gamification, cur, fx }) {
  ...
  const defaultBtc = price > 0 && gamification.acc > 0 ? Number((gamification.acc / price).toFixed(6)) : 0.01;
